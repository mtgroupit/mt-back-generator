					...
					if fields[j] == options.TitleName {
						if !options.IsStruct {
							...
							if !options.IsArray {
								SQLSelect = append(SQLSelect, shared.NameSQL(options.TitleName)+"_id")
								if needFilter {
									sqlWhereParams = append(sqlWhereParams, fmt.Sprintf(`(CAST(:%s as text) IS NULL OR %s=:%s) AND (CAST(:%s as text) IS NULL OR %s<>:%s)`, column, shared.NameSQL(options.TitleName)+"_id", column, "not_"+column, shared.NameSQL(options.TitleName)+"_id", "not_"+column))
								}
							} else {
								structIsArr = true
							}
